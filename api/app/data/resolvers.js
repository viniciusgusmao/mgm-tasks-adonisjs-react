...
const resolvers = {
  Query: {
    async fetchCompany(_, { id }) {
      const elements = await Company.query()
        .where("id", id)
    ...
    async allCustomers() {
      const elements = await Customer.query().with("company").fetch();
      return elements.toJSON();
    },
    async fetchCustomer(_, { id }) {
      const elements = await Customer.query()
        .where("id", id)
        .with("company")
        .fetch();
      return elements.toJSON()[0];
    ...
    async allProjects() {
      const elements = await Project.query()
        .with("customer")
        .with("tasks")
        .fetch();
      return elements.toJSON();
    ...
    async fetchProject(_, { id }) {
      const element = await Project.query()
        .where("id", id)
        .with("customer")
        .fetch();
    ...
    async allTasks() {
      const elements = await Task.query()
        .with("employees")
        .with("project")
        .fetch();
      return elements.toJSON();
    ...
    async fetchTask(_, { id }) {
      const element = await Task.query()
        .where("id", id)
        .with("employees")
        .with("project")
        .fetch();
    ...
    async allEmployees() {
      const elements = await Employee.query().with("tasks").fetch();
      return elements.toJSON();
    },
